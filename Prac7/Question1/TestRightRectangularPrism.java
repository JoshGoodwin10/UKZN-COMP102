...
{
    private static RightRectangularPrism pArr[] = new RightRectangularPrism[10];
    public static void main(String[] args) 
    {
        for (int i = 0; i < 10; i++)
        {
            double width = (int) (Math.random() * 11);
            double height = (int) (Math.random() * 11);
            double length = (int) (Math.random() * 11);
            pArr[i] = new RightRectangularPrism(length, height, width);
